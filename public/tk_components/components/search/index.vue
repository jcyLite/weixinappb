			...
			<div class="input">
				...
				<div class="pot-scroll-contaienr">
					<pot-scroll 
						...
						<ul class="tk-local">
								<li @click="searchVal=item" :key="index" v-for="(item,index) of local" class="tk-local-item">
...
<script>
	export default {
		...
		beforeDestroy(){
			this.blur();
			var a=localStorage.getItem('tkSearch')||'{}';
			var b=JSON.parse(a);
			b[this.path]=this.local;
			localStorage.setItem('tkSearch',JSON.stringify(b))
		},
		mounted(){
			this.path=this.$route.path||'a';
			var a=localStorage.getItem('tkSearch')||'{}';
			var b=JSON.parse(a);
			var local=b[this.path]||[];
			this.local=local;
